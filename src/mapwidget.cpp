    ...
    painters(0)
{
    ...
    renderer = new QSvgRenderer(QString(":/files/hillFlatLod1_res.svg"), this);//QString("../../hillFlat_res.svg")//QString("../src/files/bubbles.svg")
    //---------------//
    ...
    if(scale < 0.3) // full map rastr
    {
        ...
        while(counter < numberOfClasters)
        {
            ...
            if(counter % 10 != 0)
            {
                ...
                p.drawImage(width - pivotPoint.width() * (counter % 10), height - pivotPoint.height() * 2.5 * (height / previous.size().height()), painters.at(counter));
            }
            else
            {
